...
def damerau_levenshtein(
    ...
    :param np.ndarray insert_costs: a numpy array of np.float64 (C doubles) of length 2048 (0..2047),
        where insert_costs[i] is the cost of inserting ASCII character i
    :param np.ndarray delete_costs: a numpy array of np.float64 (C doubles) of length 2048 (0..2047),
        where delete_costs[i] is the cost of deleting ASCII character i
    :param np.ndarray substitute_costs: a 2D numpy array of np.float64 (C doubles) of dimensions (2048, 2048),
        where substitute_costs[i, j] is the cost of substituting ASCII character i with
        ASCII character j
    :param np.ndarray transpose_costs: a 2D numpy array of np.float64 (C doubles) of dimensions (2048, 2048),
...
def damerau_levenshtein_list_between(list list_str1,list list_str2,
    ...
    """
    Calculates the Levenshtein distance between list_str1 and list_str2,
    provided the costs of inserting, deleting, and substituting characters.
    The costs default to 1 if not provided.

    For convenience, this function is aliased as clev.dam_lev_list_b().

    :param list_str1: first list of string
    :param list_str2: second list of string
    :param np.ndarray insert_costs: a numpy array of np.float64 (C doubles) of length 2048 (0..2047),
        where insert_costs[i] is the cost of inserting ASCII character i
    :param np.ndarray delete_costs: a numpy array of np.float64 (C doubles) of length 2048 (0..2047),
        where delete_costs[i] is the cost of deleting ASCII character i
    ...
    """
    cdef:
        Py_ssize_t x, y, z, len1=len(list_str1), len2=len(list_str2)
        unsigned int kt=0
        DTYPE_t result

    cdef unsigned int* indexList = <unsigned int*> malloc(len2 * sizeof(unsigned int))
    cdef double* diss = <double*> malloc(len2 * sizeof(double))
    ...
    for x in range(len1):
        ...
        s1 = list_str1[x].encode('utf-8').decode('utf-8')
        slen1 = len(s1)
        intarr1 = convert_string_to_int_array(list_str1[x], slen1)

        for y in range(len2):

            s2 = list_str2[y].encode('utf-8').decode('utf-8')
            slen2 = len(s2)
            intarr2 = convert_string_to_int_array(list_str2[y], slen2)

            result =  c_damerau_levenshtein_unicode(
                                intarr1, slen1,
                                intarr2, slen2,
                                insert_costs,
                                delete_costs,
...
def damerau_levenshtein_list(list list_str1,list list_str2,
    ...
    For convenience, this function is aliased as clev.dam_lev_list().
...
def optimal_string_alignment_list_between(list list_str1,list list_str2,
    DTYPE_t maxv,
    DTYPE_t minv,
    DTYPE_t[::1] insert_costs=None,
    DTYPE_t[::1] delete_costs=None,
    ...
    For convenience, this function is aliased as clev.osa_list_b().
    ...
    for x in range(len1):
        ...
        for y in range(len2):
            ...
            result =  c_optimal_string_alignment_unicode(
                                intarr1, slen1,
                                intarr2, slen2,
                                insert_costs,
                                delete_costs,
...
def optimal_string_alignment_list(list list_str1,list list_str2,
    ...
    For convenience, this function is aliased as clev.osa_list().
...
def levenshtein_list(list list_str1,list list_str2,
    DTYPE_t[::1] insert_costs=None,
    DTYPE_t[::1] delete_costs=None,
    DTYPE_t[:,::1] substitute_costs=None):
    ...
    For convenience, this function is aliased as clev.lev_list().

    :param list_str1: first list of string1
    :param list_str2: second list of string2
    :param np.ndarray insert_costs: a numpy array of np.float64 (C doubles) of length 2048 (0..2047),
        where insert_costs[i] is the cost of inserting ASCII character i
    :param np.ndarray delete_costs: a numpy array of np.float64 (C doubles) of length 2048 (0..2047),
        where delete_costs[i] is the cost of deleting ASCII character i
...
def levenshtein_list_between(list list_str1,list list_str2,
    ...
    For convenience, this function is aliased as clev.lev_list_b().
