...
from weighted_levenshtein_list import dam_lev, lev, osa, dam_lev_list,lev_list


class TestClev(unittest.TestCase):

    def setUp(self):
        self.iw = np.ones(2048, dtype=np.float64)
        self.dw = np.ones(2048, dtype=np.float64)
        self.sw = np.ones((2048, 2048), dtype=np.float64)
        self.tw = np.ones((2048, 2048), dtype=np.float64)
...
class TestClevUsingDefaultValues(unittest.TestCase):
    ...
    def test_levl(self):
        self.assertEqual(list(lev_list(['a','b'], ['ab','ba'])), [('a', [['ab', 1.0], ['ba', 1.0]]), ('b', [['ab', 1.0], ['ba', 1.0]])])
        self.assertEqual(list(lev_list(['a','aa'], ['','a'])), [('a', [['', 1.0], ['a', 0.0]]), ('aa', [['', 2.0], ['a', 1.0]])])
        self.assertEqual(list(lev_list(['abc','cba'], ['ab','bc'])), [('abc', [['ab', 1.0], ['bc', 1.0]]), ('cba', [['ab', 2.0], ['bc', 2.0]])])
        self.assertEqual(list(lev_list(['abcd','abc'], ['bcd','dcb'])), [('abcd', [['bcd', 1.0], ['dcb', 3.0]]), ('abc', [['bcd', 2.0], ['dcb', 3.0]])])
        self.assertEqual(list(lev_list(['a','b'], ['ab','ba'])), [('a', [['ab', 1.0], ['ba', 1.0]]), ('b', [['ab', 1.0], ['ba', 1.0]])])

    def test_dll(self):
        self.assertEqual(list(dam_lev_list(['a','b'], ['ab','ba'])), [('a', [['ab', 1.0], ['ba', 1.0]]), ('b', [['ab', 1.0], ['ba', 1.0]])])
        self.assertEqual(list(dam_lev_list(['a','aa'], ['','a'])), [('a', [['', 1.0], ['a', 0.0]]), ('aa', [['', 2.0], ['a', 1.0]])])
        self.assertEqual(list(dam_lev_list(['abc','cba'], ['ab','bc'])), [('abc', [['ab', 1.0], ['bc', 1.0]]), ('cba', [['ab', 2.0], ['bc', 2.0]])])
        self.assertEqual(list(dam_lev_list(['abcd','abc'], ['bcd','dcb'])), [('abcd', [['bcd', 1.0], ['dcb', 3.0]]), ('abc', [['bcd', 2.0], ['dcb', 2.0]])])
        self.assertEqual(list(dam_lev_list(['a','b'], ['ab','ba'])), [('a', [['ab', 1.0], ['ba', 1.0]]), ('b', [['ab', 1.0], ['ba', 1.0]])])
